...
function get_zipped_colors(f0, f1, length, brightness=0) {
  length0 = Math.ceil(length/2);
  length1 = Math.floor(length/2);
  colors0 = f0(length0, brightness);
  colors1 = f1(length1, brightness).reverse();
  zipped_colors = [];
  for (i = 0; i < colors0.length; i++) {
...
function get_bar_green(length) {
  return get_zipped_colors(get_green_steps, get_grey_steps, length, brightness=.5);
}

function get_pie_green(length) {
  return get_zipped_colors(get_green_steps, get_grey_steps, length, brightness=.2);
}